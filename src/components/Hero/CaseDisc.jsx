...
import { CASE_SPIN, createTumble } from './discSpin';
...
const CaseDisc = forwardRef(function CaseDisc(_props, ref) {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // the case's own idle tumble — 3-axis (X/Y/Z), its own
            // independent direction/rate from discSpin.js's CASE_SPIN
            // (separate from the logo's own LOGO_SPIN, not a shared pair).
            // Runs the whole time, independent of the CSS bob on the
            // wrapper divs and of Hero's scroll timeline, which only ever
            // touches scale/z/opacity on the image group (and rotationX/Y
            // on discRef, a different element) during the fly-apart.
            const caseTumble = createTumble(caseImgGroupRef.current, CASE_SPIN);
